    ...
    static { // Changes the Look & Feel setting the System one.
			 // ie: if the user use Fedora, the frame will look like standard Fedora frames.
	...
	//System.out.println(Resultat.substring(1,2));
	//System.out.println(Resultat.substring(2,3));
	//System.out.println(Resultat.substring(3,4));
  ...
  else if(color.equals("O"))  number=1;
...
return number;
    ...
    else if(boules[i]== Color.orange) proposition+="B";
    ...
    //g.fillOval(10,460,40,40);
    //g.fillOval(55,460,40,40);
    //g.fillOval(100,460,40,40);
    //g.fillOval(145,460,40,40);
    
    for(int i=0;i<solution.length();i++)
    {
    if(solution.charAt(i)=='N') g.setColor(Color.black) ;
    else if(solution.charAt(i)=='O') g.setColor(Color.orange) ;
    ...
    else if(solution.charAt(k)=='O') g.setColor(Color.orange) ;
    ...
    g.setColor(Color.black);
    g.drawString("Bravo c'est gagné!",225,230);
	if(cestGagneA)
	{
